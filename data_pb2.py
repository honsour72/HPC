...
# Данный скрипт является основынм регулирующим логику gRPC проткокола после самого файла .proto
# Здесь при помощи класса descriptor модуля protobuf библиотеки google создаётся "ОПИСЫВАТЕЛЬ" (DESCRIPTOR) при помощи
# которго описываются и регистрируются основные сущности проекта, такие как ЗАПРОС (REQUEST), ОТВЕТ (RESPONSE),
# СЕРВИС (GRPCDEMO)
...
DESCRIPTOR = _descriptor.FileDescriptor(
    ...
    serialized_pb=_b(
        ...
        '\x43lientStreamingMethod\x12\r.demo.Request\x1a\x0e.demo.Response('
        '\x01\x12\x38\n\x15ServerStreamingMethod\x12\r.demo.Request\x1a\x0e.demo.Response0\x01\x12\x41\n\x1c'
        '\x42idirectionalStreamingMethod\x12\r.demo.Request\x1a\x0e.demo.Response(\x01\x30\x01\x62\x06proto3')
)
